...
window.validateUser = function () {
    ...
    if (user) {
        return Promise.resolve({ loggedInUser: user });
    } else {
...
window.displayUserDetails = function () {
    ...
    if (loggedInUser && userType) {
        ...
        document.getElementById('user-detail').innerHTML =
            `<h6 class="mb-0 text-primary">請先登入</h6>`;
    }
};

// Function to get the username from the user object
window.getLoggedInUserDetails = function (user) {
    let username, userId;
    switch (localStorage.getItem('userType')) {
        case "STUDENT":
            userId = user.student.studentId;
