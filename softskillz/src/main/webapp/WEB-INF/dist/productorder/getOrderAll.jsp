            ...
            <div class="sidebar-wrapper active">
                <div class="sidebar-header position-relative">
                    <div class="d-flex justify-content-between align-items-center">
                        <!-- 左上角Logo -->
                        <div class="logo">
                            <a href="/softskillz/newhomepage"><img src="/assets/compiled/jpg/logo1.jpg" alt="Logo" srcset="" /></a>
                        </div>
                        <!-- 切換日間夜間模式 -->
                                    ...
                                    <table class="table" id="orders">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><%=currentOrder.getStudentId()%></td>
                                                <td><%=currentOrder.getCouponId() != null ? currentOrder.getCouponId() : "無"%></td>
                                                <td><%=currentOrder.getOrderDate() != null ? currentOrder.getOrderDate().format(formatter) : "未設定"%></td>
                                                <td><%=currentOrder.getTotalAmount()%></td>
                                                <td><%=currentOrder.getOrderStatus()%></td>
                                                <td><%=currentOrder.getPaymentMethod()%></td>
                                                <td><%=currentOrder.getShipmentDate() != null ? currentOrder.getShipmentDate().format(formatter) : "未出貨"%></td>
                                                <td><%=currentOrder.getShipmentStatus() != null ? currentOrder.getShipmentStatus() : "未出貨"%></td>
                                                <td><%=currentOrder.getShippingAddress()%></td>
                                                <td>
                                                    <form method="get" action="/order/searchforupdate/<%=currentOrder.getOrderId()%>" onsubmit="return confirm('您確定要變更訂單嗎?');">
                                                        <button type="button" class="btn btn-primary edit-btn" data-order-id="<%=currentOrder.getOrderId()%>" data-bs-toggle="modal" data-bs-target="#updateOrderModal">
                                                            <i class="bi bi-pencil-square"></i>
                                                        </button>
                                                    </form>
                                                    <form method="post" action="/order/delete/<%=currentOrder.getOrderId()%>">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="student"
                                    name="student_id" value="${order.studentId}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="coupon" class="form-label">優惠券編號:</label>
                                <input type="text" class="form-control" id="coupon" name="coupon_id"
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="orderStatus" name="order_status">
                                    ...
                                    <option value="支付成功"
                                        ${order.orderStatus == '支付成功' ? 'selected' : ''}>支付成功</option>
                                    <option value="支付失敗"
                                        ${order.orderStatus == '支付失敗' ? 'selected' : ''}>支付失敗</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="paymentMethod" name="payment_method">
                                    <option value="">請選擇</option>
                                    <option value="綠界"
                                        ${order.paymentMethod == '綠界' ? 'selected' : ''}>綠界</option>
                                    <option value="藍新(智富通)"
                                        ${order.paymentMethod == '藍新(智富通)' ? 'selected' : ''}>藍新(智富通)</option>
                                    <option value="LINE PAY"
                                        ${order.paymentMethod == 'LINE PAY' ? 'selected' : ''}>LINE
                                        PAY</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control" id="shipmentDate"
                                    name="shipment_date" value="${order.shipmentDate}">
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="shipmentStatus"
                                    ...
                                    <option value="">請選擇</option>
                                    <option value="已出貨"
                                        ${order.shipmentStatus == '已出貨' ? 'selected' : ''}>已出貨</option>
                                    <option value="處理運送中"
                                        ${order.shipmentStatus == '處理運送中' ? 'selected' : ''}>處理運送中</option>
                                    <option value="未出貨"
                                        ${order.shipmentStatus == '未出貨' ? 'selected' : ''}>未出貨</option>
                                </select>
                            </div>
                            <div class="mb-3">
                ...
                $('.edit-btn').click(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(order) {
                            ...
                            $('#updateOrderModal #student').val(order.studentId);
                            $('#updateOrderModal #coupon').val(order.couponId);
